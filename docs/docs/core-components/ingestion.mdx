...
During ingestion, documents are broken into smaller chunks of content that are then embedded using your selected [embedding model](/ingestion-configure#set-the-embedding-model-and-dimensions).
Then, the chunks, embeddings, and associated metadata (which connects chunks of the same document) are stored in your OpenSearch knowledge base.

To modify chunking behavior and other ingestion settings, see [Configure ingestion](/ingestion-configure) and [Inspect and modify flows](/agents#inspect-and-modify-flows).
