...
2. Fetch and apply the latest container images while preserving your OpenRAG data:
   ...
   However, you can pin the `OPENRAG_VERSION` in your `.env` file to a specific version tag.
      
   If you run OpenRAG using `make` commands or manual `docker compose` commands, the `OPENRAG_VERSION` variable is used exactly as defined in the `.env` file.
   This allows you to pin the deployment to a specific version when managing the services manually.
